...
const virtual_node = defineComponent({
  ...
  setup(props, ctx) {
    ...
    async function name() {
      ...
      // 提交表单-添加新角色
      let config = { method: 'post', url: `/controller_MAIN/role/save_role?`, data: form }
      console.log('config :', config)
      let res = await axios_api(config)
      console.log('res   :', res)
      if (res.data.code == 200) {
          ...
          <ElDialog v-model={show} title="新增角色" width="400px" draggable>
            <ElForm v-model={form} label-width="80px" label-position="left">
              ...
              <ElFormItem label="菜单详情">
                <ElTree ref={ref_ElTree} style="max-width: 800px" data={menu_tree} show-checkbox node-key="menu" props={{ children: 'children', label: 'menu' }} default-expand-all default-checked-keys={form.checked_menu_list}></ElTree>
              </ElFormItem>
            </ElForm>
...
export default function role_edit_dialog({ data, that }) {
  dom_open({ ui_id: 'role_edit_dialog', virtual_node, data, that })
}
