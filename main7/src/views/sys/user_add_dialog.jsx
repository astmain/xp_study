...
const virtual_node = defineComponent({
  ...
  setup(props, ctx) {
    ...
    async function find_role_list() {
      let config = { method: 'get', url: `/controller_MAIN/role/find_role_list_str` }
      console.log('config :', config)
      let res = await axios_api(config)
      console.log('res.data   :', res.data)
      if (res.data.code == 200) {
        form.role_list = res.data.role_list
      }
    }
    ...
    return () => {
      return (
        <>
          <ElDialog v-model={show} title="新增用户" width="400px" draggable>
              ...
              <div style="width:100%; display: flex; justify-content: flex-end; ">
                <ElButton
                  ...
                  onclick={async () => {
                    ...
                    let config = { method: 'post', url: `/controller_MAIN/user/add_user`, data: form }
                    console.log('config :', config)
                    let res = await axios_api(config)
                    console.log('res.data   :', res.data)
                    if (res.data.code == 200) {
