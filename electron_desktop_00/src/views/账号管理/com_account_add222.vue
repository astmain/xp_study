...
async function get_localStorage({webview}) {
  ...
  let store = await webview_old.executeJavaScript(`(${
      function () {
...
async function copy_localStorage({webview_old, webview_new}) {
  // get===============================================
  let store = await webview_old.executeJavaScript(`(${() => window.localStorage})()`)
  console.log(`get_localStorage---成功:`, store)
  // set===============================================
  let isok = await webview_new.executeJavaScript(`(${
      function (store) {
  ...
  if (isok === true) {
    ...
    // alert("失败:copy_localStorage")
    throw Error("失败:copy_localStorage")
  }
...
export default {
  ...
  methods: {
    ...
    async 确定登录() {
      ...
      // const cookies = await session.fromPartition(partition).cookies.get({url: url_home})
      ...
      let isok_store = await copy_localStorage({webview_old: this.curr_webview, webview_new: new_webview})
