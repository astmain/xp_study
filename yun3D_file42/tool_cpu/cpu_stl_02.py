...
def cpu_stl_02(path_file):
    ...
    vectors_gpu = cp111.asarray(vectors, dtype=cp111.float32)
    parse_result = {"triangles": len(vectors)}
    print("vectors_gpu---:", vectors_gpu)
    print("parse_result---:", parse_result)

    print(f"=======长宽高=====================================================")
    min_vertex = cp111.min(vectors_gpu, axis=(0, 1))
    max_vertex = cp111.max(vectors_gpu, axis=(0, 1))
    ...
    parse_result["surface"] = float(total_area)
    ...
    with open(path_file, "rb") as f:
        ...
        parse_result["points"] = unique_vertices.size
