...
function get_path() {
    const path = require('path')
    const fs = require('fs')
    let path_project = app.getPath("userData")
    console.log(`路径---项目---path_project:`, path_project)
    let path_curr = __dirname
    console.log(`路径---当前---path_curr:`, path_curr)
    let path_log = path.join(path_project, "rebot.png")
    console.log(`路径---图表---path_log:`, path_log)
    fs.copyFileSync(path.join(path_curr, "../public/rebot.png"), path_log)
    return {path_log, path_project, path_curr}
}
...
module.exports = async function run_main_window() {
    ...
    await app.whenReady()
    // app.on('activate', () => BrowserWindow.getAllWindows().length === 0 ? app.quit() : 0)
    ...
    await mainWindow.loadURL(`http://127.0.0.1:22222?t_time=${g_time}`, {userAgent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.5359.215 Safari/537.36'})
    ...
    app.preload = path.join(__dirname, "./preload.js")
    app.tb_account = require("../prisma/tb_account")
    app.api_platform = require("tool_js_api_platform")
    app.__path = require("path")
    app.fs = require("fs")
    app.fs_extra = require("fs-extra")
    app.exec = require('child_process').exec

    app.path_preject = app.getPath("userData")
    app.path_Partitions = path.join(app.path_preject, "Partitions")
