...
let config = {
    ...
    webPreferences: {
        ...
        contextIsolation: false, //关闭警告信息,Electron 会在渲染进程中创建一个独立的 JavaScript 执行上下文(context)。这个独立的上下文中不会共享 Node.js 的全局对象和模块,比如 require、process、global 等。渲染进程中的代码只能访问浏览器提供的 Web APIs,而不能访问 Node.js 的功能
...
module.exports = async function run_app() {
    await app.whenReady()
    ...
    await globalThis.main_window.webContents.openDevTools()
    await globalThis.main_window.loadURL(`http://127.0.0.1:22222?now=${Date.now()}`, {userAgent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.5359.215 Safari/537.36'})
    winState.manage(globalThis.main_window)
